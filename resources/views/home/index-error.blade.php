...
<?php
  $user = Auth::id();
  $koneksi  = mysqli_connect('localhost', 'root', '', 'database_ziaga');
  $reco = mysqli_query($koneksi, "SELECT k.id FROM views v,categories k WHERE visitor=$user AND v.viewable_id=k.id");
  $acak = mysqli_query($koneksi, "SELECT id FROM contents WHERE category_id=1");
  while($row = mysqli_fetch_assoc($acak)){
    $test[] = reset($row);
  }
  echo json_encode($test);
  $list = json_encode ($test[array_rand($test)]);
  $hasil = str_replace(array('"','"'), '',$list);
...
<div style="margin:0 auto">
  ...
  <br>
  <a href="/post/{{$hasil}}" style="margin-top:20px" class="genric-btn primary-border circle arrow">Cek Rekomendasi</a>
</div>
    </div>
  </div>
</section>
